...
func TestIterators(t *testing.T) {
	...
	*item = 1
	Then(
		t, "typedScanner 创建并接收具体类型值",
		ExpectDo(func() error { return iter.Next(item) }),
	)
	Then(
		t, "typedScanner 回调收到扫描值",
	...
	Then(
		t, "slice 目标创建 SliceScanIterator",
