...
import (
	...
	dalcomposetarget "github.com/octohelm/storage/deprecated/pkg/dal/compose/target"
...
func (a *Action[M]) Query(patchers ...querierpatcher.Typed[M]) Result[M] {
	return &wrapper[M]{
		recvFunc: func(ctx context.Context, recv func(v *M) error) error {
...
type wrapper[M sqlbuilder.Model] struct {
	recvFunc scanner.RecvFunc[M]
}

func (r *wrapper[M]) Item(ctx context.Context) iter.Seq2[*M, error] {
	return r.recvFunc.Items(ctx)
}
