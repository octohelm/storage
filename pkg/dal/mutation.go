...
func Prepare[T any](v *T, zeroFieldsIncludes ...sqlbuilder.Column) Mutation[T] {
	return &mutation[T]{
		target:             v,
...
func (c *mutation[T]) Save(ctx context.Context, s Session) error {
	if c.forDelete {
		return c.delete(ctx, s.T(c.target), s)
	}
...
func (c *mutation[T]) delete(ctx context.Context, t sqlbuilder.Table, s Session) error {
	...
	if c.feature.softDelete {
		if soft, ok := any(c.target).(ModelWithSoftDelete); ok {
			...
			vv := soft.SetSoftDelete()
			stmt = sqlbuilder.Update(t).Where(where, additions...).Set(t.F(f).ValueBy(vv))
		}
	}
