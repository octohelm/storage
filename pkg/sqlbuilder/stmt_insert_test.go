...
func TestStmtInsert(t *testing.T) {
	...
	t.Run("insert with modifier", func(t *testing.T) {
		testingx.Expect[sqlfrag.Fragment](t,
			...
			testutil.BeFragment("INSERT IGNORE INTO t_x (f_a,f_b) VALUES (?,?)", 1, 2))
	})
	...
	t.Run("insert simple", func(t *testing.T) {
		testingx.Expect[sqlfrag.Fragment](t,
			Insert().
				...
				Values(Cols("f_a", "f_b"), 1, 2),
			testutil.BeFragment(`
INSERT INTO t_x (f_a,f_b) VALUES (?,?)
/* Comment */
`, 1, 2))
	})
	...
	t.Run("multiple insert", func(t *testing.T) {
		testingx.Expect[sqlfrag.Fragment](t,
			...
			testutil.BeFragment("INSERT INTO t_x (f_a,f_b) VALUES (?,?),(?,?),(?,?)", 1, 2, 1, 2, 1, 2))
	})
	...
	t.Run("insert from select", func(t *testing.T) {
		testingx.Expect[sqlfrag.Fragment](t,
			...
			testutil.BeFragment(`
INSERT INTO t_x (f_a,f_b) SELECT f_a,f_b FROM t_x
WHERE f_a = ?
`, 1))
	})
}
