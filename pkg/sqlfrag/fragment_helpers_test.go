...
func TestConstFuncAndContext(t *testing.T) {
	...
	Then(
		t, "Const 和 Empty 暴露基本 fragment 行为",
		Expect(q, Equal("x")),
	...
	Then(
		t, "Func 和 WithContextInjector 允许定制上下文输出",
...
func TestBlockArgAndOperators(t *testing.T) {
	...
	Then(
		t, "Map 和 BlockWithoutBrackets 支持片段序列展开",
		Expect(q, Equal("?,?")),
	)
	...
	Then(
		t, "Join(nil) 与 Empty 都是空 fragment",
